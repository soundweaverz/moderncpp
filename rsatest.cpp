...
int rsaencrypt(RSA *rsa, EVP_PKEY *pkey, std::string cleantext)
{
    std::string ciphertext {};

    RSA_private_encrypt(cleantext.size(), (unsigned char*) cleantext, (unsigned char*) ciphertext, rsa, 0);
...
int main(int argc, const char* argv[])
{
    ...
    if(!rsaencrypt(rsa.get(), pkey.get(), text))
    {
        return 1;
    }
